---
title: "Testing Smart Contract Using Hardhat and Waffle"
date: 2022-02-22
slug: testing-smart-contract-hardhat-waffle
description: "Setup Hardhat and Waffle to test a smart contract. We use Mocha and Chai to perform the test"
tags: ["web"]
---
...
In the previous article, [How To Create Your First Smart Contract](https://backbencher.dev/articles/how-to-create-first-smart-contract), we created our first smart contract. Next step is to test it.
...
`@nomiclabs/hardhat-waffle` is a hardhat plugin for integration with `waffle`. [Waffle](https://getwaffle.io/) is a tool for smart contract testing.
...
Next, open `hardhat.config.ts` file and add following 2 import statements for waffle package and ether package.
...
First line picks the `HelloWorld` contract which we created in the [previous article](https://backbencher.dev/articles/how-to-create-first-smart-contract).

Second line deploys the contract to a network. In this case, the network is a test network by Waffle.
