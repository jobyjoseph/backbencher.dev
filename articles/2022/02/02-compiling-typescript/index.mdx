...
date: 2022-02-02
slug: compiling-typescript
description: "The goal of this article is to learn how to compile a TypeScript program. We use tsc compiler command to perform the compilation. We also learn how to set the level of JavaScript that comes out of TypeScript compiler."
tags: ["typescript"]
---

The goal of this article is to learn how to **compile** a TypeScript program. We use `tsc` compiler command to perform the compilation. We also learn how to set the **level of JavaScript** that comes out of TypeScript compiler.
...
The `dev` command will run the TypeScript compiler(`tsc`) along with some options. `--watch` flag watches for any file change and does compilation again. `--preserveWatchOutput` is to retain the console outputs even after re-compilation.
...
By mentioning `"src"` in `include` array, we specify that all our TypeScript files will be in `src` folder.
...
For example, in the above TypeScript code if we called the function with a string value like below, TypeScript can identify the error at build time itself.
